 ...
 */
AP_Proximity_uSharp3D_M::AP_Proximity_uSharp3D_M(AP_Proximity &_frontend,
		AP_Proximity::Proximity_State &_state, AP_SerialManager &serial_manager) :
		AP_Proximity_Backend(_frontend, _state) {
	Utility::my_fn = (char*)"uSharp3D_M";
	...
	for (uint8_t i = 0; i < _num_sectors; i++) {
		...
		_distance[i] = PROXIMITY_USHARP3D_M_DISTANCE_MAX;
	...
	avoid_dis = PROXIMITY_USHARP3D_M_DISTANCE_MAX;
...
bool AP_Proximity_uSharp3D_M::detect(AP_SerialManager &serial_manager) {
	//
...
void AP_Proximity_uSharp3D_M::update(void) {
//	put_sending();
	if (get_reading()) {
...
bool AP_Proximity_uSharp3D_M::put_sending(void) {
	if (uart == nullptr) {
		return false;
	}
...
bool AP_Proximity_uSharp3D_M::get_reading(void) {
	if (uart == nullptr) {
		return false;
	}
...
//						angleV_data[j] = agl_v;
	...
	if (avoid_dis > 2) {
		...
		_distance[0] = PROXIMITY_USHARP3D_M_DISTANCE_MAX;
	}
...
float AP_Proximity_uSharp3D_M::distance_max() const {
	return PROXIMITY_USHARP3D_M_DISTANCE_MAX;
}

float AP_Proximity_uSharp3D_M::distance_min() const {
	return PROXIMITY_USHARP3D_M_DISTANCE_MIN;
}
