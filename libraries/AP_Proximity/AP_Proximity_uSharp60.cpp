 ...
 */
AP_Proximity_uSharp60::AP_Proximity_uSharp60(AP_Proximity &_frontend,
		AP_Proximity::Proximity_State &_state, AP_SerialManager &serial_manager) :
		AP_Proximity_Backend(_frontend, _state) {
	Utility::my_fd_name = (char*)"uSharp60";
	...
	for (uint8_t i = 0; i < _num_sectors; i++) {
		...
		_distance[i] = PROXIMITY_USHARP60_DISTANCE_MAX;
		_distance_valid[i] = true;
	}

	memset(buf, 0, 1024);
	buf_idx = 0;
	avoid_dis = PROXIMITY_USHARP60_DISTANCE_MAX;
...
bool AP_Proximity_uSharp60::detect(AP_SerialManager &serial_manager) {
	//
...
void AP_Proximity_uSharp60::update(void) {
	put_sending();
	if (get_reading()) {
...
bool AP_Proximity_uSharp60::put_sending(void) {
	if (uart == nullptr) {
		return false;
	}
...
bool AP_Proximity_uSharp60::get_reading(void) {
	if (uart == nullptr) {
		return false;
	}
	...
	while (nbytes-- > 0) {
		...
		if (buf_idx == 0) {
			...
			if (buf_idx == 11) {
				...
				{
					hal.console->printf("uknown cmd or subcmd %d %d. \n", buf[3],buf[4]);
					buf_idx = 0;
				}
				else {
	...
	if (avoid_dis > 2) {
		...
		_distance[0] = PROXIMITY_USHARP60_DISTANCE_MAX;
	}
...
float AP_Proximity_uSharp60::distance_max() const {
	return PROXIMITY_USHARP60_DISTANCE_MAX;
}

float AP_Proximity_uSharp60::distance_min() const {
	return PROXIMITY_USHARP60_DISTANCE_MIN;
}
