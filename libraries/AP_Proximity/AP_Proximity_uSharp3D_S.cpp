 ...
 */
AP_Proximity_uSharp3D_S::AP_Proximity_uSharp3D_S(AP_Proximity &_frontend,
		AP_Proximity::Proximity_State &_state, AP_SerialManager &serial_manager) :
		AP_Proximity_Backend(_frontend, _state) {
	Utility::my_fn = (char*)"uSharp3D_S";
	...
	for (uint8_t i = 0; i < _num_sectors; i++) {
		...
		_distance[i] = PROXIMITY_USHARP3D_S_DISTANCE_MAX;
		_distance_valid[i] = true;
	}

	memset(buf, 0, 256);
	buf_idx = 0;
	avoid_dis = PROXIMITY_USHARP3D_S_DISTANCE_MAX;
...
bool AP_Proximity_uSharp3D_S::detect(AP_SerialManager &serial_manager) {
	//
...
void AP_Proximity_uSharp3D_S::update(void) {
//	put_sending();
	if (get_reading()) {
...
bool AP_Proximity_uSharp3D_S::put_sending(void) {
	if (uart == nullptr) {
		return false;
	}
...
bool AP_Proximity_uSharp3D_S::get_reading(void) {
	if (uart == nullptr) {
		return false;
	}
...
//						angleV_data[j] = agl_v;
	...
	if (avoid_dis > 2) {
		...
		_distance[0] = PROXIMITY_USHARP3D_S_DISTANCE_MAX;
	}
...
float AP_Proximity_uSharp3D_S::distance_max() const {
	return PROXIMITY_USHARP3D_S_DISTANCE_MAX;
}

float AP_Proximity_uSharp3D_S::distance_min() const {
	return PROXIMITY_USHARP3D_S_DISTANCE_MIN;
}
