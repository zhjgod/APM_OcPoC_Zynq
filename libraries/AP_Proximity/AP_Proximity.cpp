...
void AP_Proximity::detect_instance(uint8_t instance)
{
	...
	if (type == Proximity_Type_uLandingSt_by) {
        state[instance].instance = instance;
        drivers[instance] = new AP_Proximity_uLandingSt_by(*this, state[instance], serial_manager);
        return;
    }
	...
	if (type == Proximity_Type_usharp60_mncan) {
 	...
 	if (type == Proximity_Type_usharp60_sc) {
        state[instance].instance = instance;
        drivers[instance] = new AP_Proximity_usharp60_sc(*this, state[instance], serial_manager);
		return;
	}
